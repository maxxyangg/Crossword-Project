...
public class CrosswordBoard {
	...
	public void findBoardSize() {
		board = new Letter[Preferences.TABLE_ROW][Preferences.TABLE_COL];

	}

	public void fillBoardWithSpaces() {
		Word blankWord = new Word("Blank", new ArrayList<Letter>(), -1, -1, "No where", "None", -1);
		for (int i = 0; i < Preferences.TABLE_ROW; i++) {
			for (int col = 0; col < Preferences.TABLE_COL; col++) {
				Letter underScore = new Letter(i, col, " ", blankWord);
				board[i][col] = underScore;
			}
		}
	}

	public boolean insertToBoard(ArrayList<Letter> logicalChars, Word word, int clueNumber) {
		...
		if (wordsOnBoard.isEmpty()) {
			if ((Preferences.TABLE_COL / 2) - 4 + word.getWord().length() < Preferences.TABLE_COL) {
				...
				word.setRow((Preferences.TABLE_ROW / 2) - 1);
				word.setCol((Preferences.TABLE_COL / 2) - 4);
				word.setDirection("Horizontal");
				word.setClueNumber(clueNumber);
			...
			if (!(findMatch(logicalChars, word) == null)) {
				...
				if (isValid(logicalChars, word, match, direction, matchedIndex)) {
					...
					if (direction.equalsIgnoreCase("Vertical")) {
						...
						word.setRow(match.getRow() - matchedIndex);
						...
						word.setCol(match.getColumn() - matchedIndex);
						word.setClueNumber(clueNumber);
						wordsOnBoard.add(word);
						numberOfWords++;
					}
